...
class GetDifference {
	...
	var temp,len;final flags = reader.readInt();var pts = reader.readInt();var ptsTotalLimit;if ((flags & 1)==1){ptsTotalLimit = reader.readInt(); } else {ptsTotalLimit=null;}var date = reader.readInt();var qts = reader.readInt();		return new GetDifference(pts : pts, ptsTotalLimit : ptsTotalLimit, date : date, qts : qts);
	}
	List<int> getBytes(){return [readBufferFromBigInt(630429265,4),[0,0,0,0],readBufferFromBigInt(this.pts,4,little:true,signed:true),(this.ptsTotalLimit==null||this.ptsTotalLimit==false)?new List<int>.empty():[readBufferFromBigInt(this.ptsTotalLimit,4,little:true,signed:true)].expand((element) => element).toList(),readBufferFromBigInt(this.date,4,little:true,signed:true),readBufferFromBigInt(this.qts,4,little:true,signed:true),].expand((element) => element).toList();}
	readResult(BinaryReader reader) {
	return reader.tgReadObject();
	}
