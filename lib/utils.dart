...
Map<String, List<int>> generateKeyDataFromNonce(serverNonce, newNonce) {
  ...
  final List<int> hash1 = sha1
      .convert(
          [newNonceList, serverNonceList].expand((element) => element).toList())
      .bytes;
  final List<int> hash2 = sha1
      .convert(
          [serverNonceList, newNonceList].expand((element) => element).toList())
      .bytes;
  final List<int> hash3 = sha1
      .convert(
          [newNonceList, newNonceList].expand((element) => element).toList())
      .bytes;
...
List<int> toSignedLittleBuffer(BigInt big, {int number: 8}) {
  final byteArray = new List.filled(number, 0);
...
List<int> serializeBytes(dynamic data) {
  ...
  final s = new List.filled(padding, 0);
  r.addAll(s);
